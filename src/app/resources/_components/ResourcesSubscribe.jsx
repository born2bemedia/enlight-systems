...
function ResourcesSubscribe() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await submitForm("/api/subscribe", { email: value });
      setSent(true);
    } catch {
  ...
  return (
    <section className="resources-subscribe">
      <div className="_container">
        <div className="resources-subscribe__panel">
          <h2>Get notified when the new articles are coming!</h2>
          {sent ? (
            <p className="resources-subscribe__success">
              Thanks! We&apos;ll notify you when new articles are published.
            </p>
          ) : (
            <form className="resources-subscribe__form" onSubmit={handleSubmit}>
              <div
                className={`resources-subscribe__field${
                  error ? " resources-subscribe__field--error" : ""
                }`}
              >
                <input
                  type="email"
              ...
              {error && <p className="resources-subscribe__error">{error}</p>}
            </form>
          )}
        </div>
      </div>
...
export default ResourcesSubscribe;
